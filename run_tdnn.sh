...
if [ $stage -le 16 ]; then
  ...
  for data in $test_sets; do
    (
      ...
      #right now we don't have mutliple LMs to run rescore scripts. ALso we can use consta arpa lm rescoring once we build it. Experiment with it later. 
      steps/lmrescore.sh --cmd "$decode_cmd" \
        data/lang data/lang \
       data/${data}_hires ${dir}/decode_${data} ${dir}/decode_${data}_new || exit 1
    ) || touch $dir/.error &
  done
  wait
...
if $test_online_decoding && [ $stage -le 17 ]; then
  ...
  for data in $test_sets; do
    (
      ...
      #Experiment with const arpa lm rescoring. Currently, I have not built it. 
      steps/lmrescore.sh --cmd "$decode_cmd" \
        data/lang data/lang \
       data/${data}_hires ${dir}_online/decode_${data} ${dir}_online/decode_${data}_new || exit 1
    ) || touch $dir/.error &
  done
  wait
  [ -f $dir/.error ] && echo "$0: there was a problem while decoding" && exit 1
